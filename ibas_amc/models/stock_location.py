...
class StockLocationInherit(models.Model):
    ...
    is_amc_return_stock_location = fields.Boolean(string='Is AMC Return Stock Location?')
class StockPickingInherit(models.Model):
    _inherit = 'stock.picking'

    return_request_id = fields.Many2one(
        'return.stock.request', 'Return Request Stock')
    ...
    def _compute_scheduled_date(self):
        if self.return_request_id:
            ...
            if self.move_type == 'direct':
                self.scheduled_date = min(self.move_lines.mapped(
                    'date_expected') or [fields.Datetime.now()])
            else:
                self.scheduled_date = max(self.move_lines.mapped(
                    'date_expected') or [fields.Datetime.now()])
